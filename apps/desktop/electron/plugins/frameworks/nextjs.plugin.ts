...
export class NextJSPlugin implements ScannerPlugin<FrameworkPluginResult> {
  name = 'nextjs';
  version = '1.0.0';
  type = 'framework' as const;
  priority = 100;

  private routerType: 'app' | 'pages' | null = null;
  ...
  async analyze(context: AnalysisContext): Promise<FrameworkPluginResult> {
    ...
    const middlewareFile = sourceFiles.find(f => 
      f.relativePath === 'middleware.ts' || f.relativePath === 'src/middleware.ts'
    );
    if (middlewareFile) {
   ...
   */
  private async extractAppRouterRoutes(
   ...
   */
  private async extractPagesRouterRoutes(
    context: AnalysisContext,
    basePath: string,
    routes: RouteInfo[]
  ): Promise<void> {
    ...
    const pageFiles = sourceFiles.filter(f => {
      const rel = f.relativePath;
      if (!rel.startsWith(basePath)) return false;
      if (rel.includes('/api/')) return false;
      if (rel.includes('_app.') || rel.includes('_document.') || rel.includes('_error.')) return false;
   ...
   */
  private isRouteProtected(routePath: string, config: AnalysisContext['config']): boolean {
   ...
   */
  private findExportedComponent(sourceFile: ts.SourceFile): string | null {
    ...
    const visit = (node: ts.Node) => {
      ...
      if (ts.isCallExpression(node) && ts.isIdentifier(node.expression)) {
        ...
        if (hookName.startsWith('use')) {
          ...
          if (hookName === 'useEffect' || hookName === 'useLayoutEffect') hasEffects = true;
          if (hookName === 'useForm') hasForms = true;
        }
      }
      ...
      if (ts.isJsxElement(node) || ts.isJsxSelfClosingElement(node)) {
        ...
        if (tagLower === 'form') hasForms = true;
      }
   ...
   */
  private extractNavigationLinks(sourceFile: ts.SourceFile, filePath: string): NavigationLink[] {
   ...
   */
  private extractServerActions(sourceFile: ts.SourceFile, filePath: string): ServerActionInfo[] {
   ...
   */
  private extractMiddleware(sourceFile: ts.SourceFile, filePath: string): MiddlewareInfo | null {
    let matcher: string[] = [];
