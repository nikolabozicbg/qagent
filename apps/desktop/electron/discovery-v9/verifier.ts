 ...
 */
export function verifyAndFilter(
  ...
  const observationById = new Map<string, ActionObservation>();
  for (const obs of observations) {
    observationById.set(obs.candidateId, obs);
  }

  const verifiedSteps: VerifiedStep[] = [];
  ...
  for (const candidate of candidates) {
    ...
    // Build verified step
    const verifiedStep: VerifiedStep = {
 ...
 */
function verifyObservableEffect(
  candidate: CandidateAction,
  observation: ActionObservation
): { reason: VerifiedStep['verificationReason'] } | null {
  ...
  if (observation.networkCalls.length > 0) {
    ...
    const hasSuccessfulCall = observation.networkCalls.some(
      c => c.status !== null && c.status >= 200 && c.status < 400
    );
 ...
 */
function hasSignificantDomChange(observation: ActionObservation): boolean {
  ...
  const significantMutations = observation.domMutations.filter(m => {
    ...
    const isNoise = noiseSelectors.some(n => 
      m.selector.toLowerCase().includes(n) ||
 ...
 */
function deduplicateByEffect(
  steps: VerifiedStep[],
  discardReasons: Record<string, number>
): VerifiedStep[] {
  const uniqueEffects = new Map<string, VerifiedStep>();
 ...
 */
function computeEffectKey(step: VerifiedStep): string {
  const parts: string[] = [];
 ...
 */
export function buildVerifiedFlows(verifiedSteps: VerifiedStep[]): VerifiedFlow[] {
  ...
  const stepsByDestination = new Map<string, VerifiedStep[]>();
  ...
  for (const [destination, steps] of stepsByDestination) {
    ...
    const bySource = new Map<string, VerifiedStep>();
    for (const step of steps) {
  ...
  const formSteps = verifiedSteps.filter(
    s => s.candidate.type === 'form-submit' && 
  ...
  const interactionSteps = verifiedSteps.filter(
    s => s.candidate.type === 'button' && 
