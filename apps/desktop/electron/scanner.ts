...
// Types matching backend AnalysisPayload
...
interface ComponentInfo {
  ...
  props: { name: string; type: string | null; isRequired: boolean }[];
  hooks: string[];
  hasState: boolean;
  hasEffects: boolean;
  hasForms: boolean;
...
interface FieldInfo {
  ...
  validations: { type: string; value: string | number | null; message: string | null }[];
  ...
  defaultValue: string | null;  // Default value if present
}

interface APIInfo {
  method: string;
  path: string;
  calledFrom: { component: string; filePath: string }[];
...
interface BehaviorInfo {
  type: string;
  description: string;
  trigger: { element: string | null; event: string; selector: string | null };
  outcome: { type: string; target: string | null };
  component: string;
  filePath: string;
}
...
interface RelationshipData {
  ...
  navigationLinks: { from: string; to: string; linkText: string | null; selector: string | null }[];
  formToEntity: Record<string, string>;
  entityToRoutes: Record<string, string[]>;
  inferredFlows: { name: string; description: string; steps: string[]; entities: string[]; importance: number }[];
}
...
export class ProjectScanner {
  ...
  private framework: ProjectInfo['framework'] = {
    name: 'unknown',
    version: '',
    router: null,
    stateManagement: []
  ...
  // Smart analysis data
  private smartForms: SmartFormInfo[] = [];
  private navigationLinks: NavigationLink[] = [];
  private testCredentials: TestCredentials[] = [];
  private routeToFilePath: Map<string, string> = new Map();

  constructor(projectPath: string) {
    this.projectPath = projectPath;
  }

  async scan(): Promise<AnalysisPayload> {
    ...
    const minedData = await testDataMiner.mineTestData(this.projectPath, this.sourceFiles);
   ...
   */
  private async runSmartAnalysis(): Promise<void> {
    ...
    for (const route of this.routes) {
      try {
        ...
        const sourceFile = ts.createSourceFile(
          ...
          true,
          route.filePath.endsWith('.tsx') ? ts.ScriptKind.TSX : ts.ScriptKind.TS
        );
        
        // Analyze forms with smart analyzer
        const forms = smartFormAnalyzer.analyzeFile(
          sourceFile, 
          route.filePath, 
          route.path,
          content
        ...
        const links = navigationFlowAnalyzer.extractNavigationLinks(
          sourceFile,
          route.path,
          componentName
        );
   ...
   */
  private linkFormsToRoutes(): void {
    ...
    for (const form of this.forms) {
      if (form.route) continue; // Already has route
      ...
      for (const route of this.routes) {
        ...
        if (isRegisterForm && isSignupRoute) score = 10;
        ...
        if (isLoginForm && isSigninRoute) score = 10;
  ...
  private mergeSmartForms(): void {
    for (const sf of this.smartForms) {
      ...
      const existingIndex = this.forms.findIndex(f => 
        f.id === sf.id || 
  ...
  private async findSourceFiles(dir: string): Promise<void> {
    try {
  ...
  private async parseFile(filePath: string, content: string): Promise<void> {
  ...
  private extractFromAST(sourceFile: ts.SourceFile, filePath: string, relativePath: string): void {
  ...
  private extractFormFromJSX(
    ...
    if (tagName === 'form') {
      ...
      let submitButton: FormInfo['submitButton'] = null;
      let submitEndpoint: string | null = null;
  ...
  private extractFormFields(node: ts.JsxElement, fields: FieldInfo[], filePath: string, parentControllerName?: string): void {
    const visit = (child: ts.Node, controllerName?: string) => {
      if (ts.isJsxElement(child) || ts.isJsxSelfClosingElement(child)) {
        ...
        if (isInputElement || isCustomInputComponent) {
          ...
          const validations: FieldInfo['validations'] = [];
          if (validationAttr) {
   ...
   */
  private extractRegisterFieldName(element: ts.JsxOpeningElement | ts.JsxSelfClosingElement): string | null {
   ...
   */
  private inferFieldNameFromPlaceholder(placeholder: string | null): string | null {
   ...
   */
  private inferFieldNameFromValue(valueAttr: ts.JsxAttribute): string | null {
    if (!valueAttr.initializer) return null;
   ...
   *   "Email Address" -> "email"
   */
  private labelToFieldName(label: string | null): string | null {
  ...
  private findSubmitButton(formNode: ts.JsxElement): FormInfo['submitButton'] {
    let submitButton: FormInfo['submitButton'] = null;
  ...
  private findJsxAttribute(element: ts.JsxOpeningElement | ts.JsxSelfClosingElement, attrName: string): ts.JsxAttribute | null {
    for (const prop of element.attributes.properties) {
  ...
  private getAttributeValue(attr: ts.JsxAttribute | null): string | null {
   ...
   */
  private getAttributeValueFromExpression(attr: ts.JsxAttribute | null): string | null {
    if (!attr || !attr.initializer) return null;
  ...
  private inferFormName(componentName: string, filePath: string): string {
    ...
    if (filePath.includes('sign-in')) return 'Login Form';
    if (filePath.includes('sign-up')) return 'Registration Form';
    if (filePath.includes('password-reset')) return 'Password Reset Form';
    
    return `${componentName} Form`;
  }

  private findSuccessRedirect(filePath: string): string | null {
  ...
  private extractInterface(node: ts.InterfaceDeclaration, filePath: string): void {
    ...
    if (name.endsWith('Props')) semanticType = 'props';
    else if (name.endsWith('State')) semanticType = 'state';
  ...
  private extractAPICall(node: ts.CallExpression, filePath: string, relativePath: string): void {
  ...
  private extractJSXElement(node: ts.JsxElement | ts.JsxSelfClosingElement, filePath: string, relativePath: string): void {
    const openingElement = ts.isJsxElement(node) ? node.openingElement : node;
  ...
  private findParentComponent(relativePath: string): string {
    // Find component that owns this file
    const matchingComponent = this.components.find(c => 
      c.filePath.includes(relativePath) || relativePath.includes(path.basename(c.filePath, path.extname(c.filePath)))
    );
    return matchingComponent?.name || path.basename(relativePath, path.extname(relativePath));
  }

  private inferInteractionType(tagName: string): string | null {
   ...
   */
  private async extractGenericJSXRoutes(): Promise<void> {
      ...
      try {
        ...
        for (const pattern of pathPatterns) {
          ...
          while ((match = pattern.exec(content)) !== null) {
            ...
            if (foundPaths.has(normalizedPath)) continue;
            foundPaths.add(normalizedPath);
            
            // Check if route already exists
            if (this.routes.some(r => r.path === normalizedPath)) continue;
   ...
   */
  private inferRouteProtection(filePath: string, routePath: string): boolean {
   ...
   */
  private findComponentNameForFile(filePath: string): string | null {
    const component = this.components.find(c => c.filePath === filePath);
    return component?.name || null;
  }

  private async extractNextRoutes(): Promise<void> {
  ...
  private async scanNextAppRouter(dir: string, routePath: string): Promise<void> {
    try {
      const entries = await fs.readdir(dir, { withFileTypes: true });
  ...
  private async scanNextPagesRouter(dir: string, routePath: string): Promise<void> {
    try {
      const entries = await fs.readdir(dir, { withFileTypes: true });
  ...
  private buildRelationships(): RelationshipData {
    const componentToTypes: Record<string, string[]> = {};
    const componentToApis: Record<string, string[]> = {};
    ...
    const navigationLinks: RelationshipData['navigationLinks'] = this.navigationLinks.map(link => ({
      from: link.from,
      to: link.to,
    ...
    const flowGraph = navigationFlowAnalyzer.buildFlowGraph(
    ...
    const graphJourneys = navigationFlowAnalyzer.inferUserJourneys(flowGraph);
    const routeFlows = this.inferFlows();
    
    // Merge journeys, prioritizing graph-based ones
    const inferredFlows: RelationshipData['inferredFlows'] = [];
  ...
  private inferEntitiesFromSteps(steps: string[]): string[] {
  ...
  private inferFlows(): RelationshipData['inferredFlows'] {
    const flows: RelationshipData['inferredFlows'] = [];
...
export async function scanProject(projectPath: string): Promise<AnalysisPayload> {
  const scanner = new ProjectScanner(projectPath);
  return scanner.scan();
}

// ============================================================================
// V5 UNIVERSAL SCANNER TYPES AND PAYLOAD GENERATION
// ============================================================================

/**
 * V5 Universal Scanner Payload Types
 * Zero enums - all strings to prevent hallucination
 * All selectors ranked, all constraints with examples
 */

export interface V5SelectorInfo {
  value: string;                    // The actual selector string
  strategy: string;                 // 'data-testid' | 'data-cy' | 'aria-label' | 'name' | 'id' | 'css'
  confidence: number;               // 0-1 confidence score
  source: string;                   // 'attribute' | 'generated' | 'inferred'
}

export interface V5ElementInfo {
  id: string;                       // Unique element identifier
  type: string;                     // 'input' | 'button' | 'link' | 'select' | 'checkbox' | 'radio' | 'textarea'
  role: string;                     // Semantic role: 'email' | 'password' | 'submit' | 'navigation' | 'form-field'
  selectors: V5SelectorInfo[];      // Ranked list of selectors
  bestSelector: string;             // Best selector for use in tests
  label: string | null;
  placeholder: string | null;
  defaultValue: string | null;
  pageId: string;                   // Which page this element belongs to
  formId: string | null;            // Which form this element belongs to (if any)
}

export interface V5ConstraintInfo {
  id: string;                       // Unique constraint identifier
  elementId: string;                // Which element this constraint applies to
  type: string;                     // 'required' | 'minLength' | 'maxLength' | 'pattern' | 'email' | 'min' | 'max' | 'custom'
  rule: string;                     // The actual constraint rule (e.g., 'minLength:3', 'pattern:^[a-z]+$')
  message: string | null;           // Error message if constraint fails
  validExamples: string[];          // Values that satisfy this constraint
  invalidExamples: string[];        // Values that violate this constraint
  source: string;                   // 'html5' | 'zod' | 'yup' | 'react-hook-form' | 'inferred'
}

export interface V5PageInfo {
  id: string;                       // Unique page identifier
  path: string;                     // Route path (e.g., '/sign-in', '/products/[id]')
  title: string;                    // Page title or inferred name
  isProtected: boolean;             // Requires authentication
  isDynamic: boolean;               // Has dynamic segments
  params: string[];                 // Dynamic route params
  elementIds: string[];             // Elements on this page
  formIds: string[];                // Forms on this page
}

export interface V5FormInfo {
  id: string;                       // Unique form identifier
  name: string;                     // Form name
  pageId: string;                   // Which page this form is on
  fieldIds: string[];               // Element IDs of form fields
  submitButtonId: string | null;    // Element ID of submit button
  submitEndpoint: string | null;    // API endpoint on submit
  successRedirect: string | null;   // Redirect URL on success
  constraintIds: string[];          // Constraint IDs for this form
}

export interface V5FlowInfo {
  id: string;                       // Unique flow identifier
  name: string;                     // Flow name (e.g., 'User Registration', 'Checkout')
  description: string;
  steps: V5FlowStep[];              // Ordered steps
  entities: string[];               // Entities involved
  importance: number;               // 0-1 importance score
  source: string;                   // 'explicit' | 'inferred' | 'navigation'
}

export interface V5FlowStep {
  order: number;
  pageId: string;                   // Page for this step
  action: string;                   // 'navigate' | 'fill-form' | 'click' | 'assert'
  targetElementId: string | null;   // Element to interact with
  description: string;
}

export interface V5ConfigInfo {
  detectedTestFramework: string | null;  // 'jest' | 'vitest' | 'playwright' | 'cypress' | null
  selectorPriority: string[];            // Ordered priority: ['data-testid', 'data-cy', 'aria-label', 'name', 'id']
  baseUrl: string | null;                // Detected base URL
  authEndpoint: string | null;           // Detected auth endpoint
}
...
export interface V5ScannerPayload {
  ...
  // Raw data for LLM context (not for direct use)
  _raw: {
    types: TypeInfo[];
    apis: APIInfo[];
    relationships: RelationshipData;
 ...
 */
function rankSelectors(field: FieldInfo, elementType: string): V5SelectorInfo[] {
  const selectors: V5SelectorInfo[] = [];
...
function extractConstraints(
  ...
  const constraints: V5ConstraintInfo[] = [];
 ...
 */
function generatePatternExamples(pattern: string, valid: boolean): string[] {
 ...
 */
function detectElementRole(field: FieldInfo): string {
  const nameLower = field.name.toLowerCase();
  
  if (field.type === 'email' || nameLower.includes('email')) return 'email';
  if (field.type === 'password' || nameLower.includes('password')) return 'password';
  if (field.type === 'submit') return 'submit';
  if (field.type === 'tel' || nameLower.includes('phone')) return 'phone';
  if (nameLower.includes('name') && nameLower.includes('user')) return 'username';
  if (nameLower.includes('first') && nameLower.includes('name')) return 'firstName';
  if (nameLower.includes('last') && nameLower.includes('name')) return 'lastName';
  if (nameLower === 'name' || nameLower.includes('fullname')) return 'fullName';
 ...
 */
export function convertToV5ScannerPayload(payload: AnalysisPayload): V5ScannerPayload {
  const pages: V5PageInfo[] = [];
  const elements: V5ElementInfo[] = [];
  const forms: V5FormInfo[] = [];
  const constraints: V5ConstraintInfo[] = [];
  const flows: V5FlowInfo[] = [];
  ...
  for (const form of payload.forms) {
    ...
    if (form.submitButton) {
      ...
      const buttonSelectors: V5SelectorInfo[] = [];
    ...
    if (selector.isInteractive && selector.interactionType === 'navigation') {
      ...
      elements.push({
        ...
        selectors: [{
          ...
          strategy: selector.strategy as string,
          confidence: 0.8,
          source: 'attribute'
        }],
  ...
  for (const inferredFlow of payload.relationships.inferredFlows) {
    ...
    const steps: V5FlowStep[] = inferredFlow.steps.map((stepPath, index) => {
  ...
  return {
    ...
    config: {
      ...
      baseUrl: null,
      authEndpoint: payload.apis.find(a => 
        a.path.includes('auth') || a.path.includes('login') || a.path.includes('sign')
      )?.path || null,
    },
    _raw: {
      types: payload.types,
 ...
 */
export async function scanProjectV5(projectPath: string): Promise<V5ScannerPayload> {
