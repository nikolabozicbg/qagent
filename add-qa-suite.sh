...
echo "🚀 Adding Enterprise QA Suite features..."
...
npx shadcn@latest add table badge -y 2>/dev/null || echo "Components already exist"

###############################################################################
# 3. CREATE QA SUITE COMPONENT
###############################################################################

echo "📝 Creating QA Suite component..."

mkdir -p src/components/qa-suite

cat > src/components/qa-suite/QASuite.tsx << 'EOF'
"use client";
...
###############################################################################
# 4. ADD QA SUITE TO RESULT PAGE
###############################################################################

echo "📝 Updating result page..."

cat > src/app/result/page-import-patch.txt << 'EOF'
import QASuite from "@/components/qa-suite/QASuite";
EOF

# Add import at the top (after existing imports)
sed -i '' '/^import { useSubscription }/a\
import QASuite from "@/components/qa-suite/QASuite";\
' src/app/result/page.tsx

# Add QA Suite before closing div
sed -i '' '/<\/Tabs>/a\
\
      {/* QA Suite Section */}\
...
echo ""
echo "✅ Enterprise QA Suite added successfully!"
echo ""
echo "Features added:"
echo "  ✓ Requirements Traceability Matrix (RTM)"
echo "  ✓ Negative & Edge-Case Tests"
echo "  ✓ Boundary Value Analysis (BVA)"
echo "  ✓ Equivalence Partitioning (EP)"
echo "  ✓ Security Test Recommendations"
echo "  ✓ API Test Suite"
echo "  ✓ Sticky sidebar navigation"
